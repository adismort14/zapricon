...
def posts_home(request, topic_slug=None):
    ...
    return render(
        request, "posts/posts_home.html", {"posts": posts, "page": page, "topic": topic}
    )
        ...
        if edit_post_form.is_valid():
            ...
            if post.status == "draft":
                ...
                return HttpResponseRedirect(
                    reverse_lazy("posts:post_detail", args=[post.pk, post.slug])
                )

    else:
        edit_post_form = EditPostForm(
            data={"title": post.title, "body": post.body, "topics": []}
        )
